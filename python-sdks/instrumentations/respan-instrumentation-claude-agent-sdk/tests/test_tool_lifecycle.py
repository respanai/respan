...
def otel():
    ...
    def context(*, capture_content=True):
        value = InvocationContext(
            tracer.start_span("invocation"), capture_content=capture_content
        )
        contexts.append(value)
        return value

    yield SimpleNamespace(
        provider=provider, recorder=recorder, tracer=tracer, context=context
    )
...
async def _hook(otel, ctx, event, tool_id, data):
    ...
    try:
        hooks = _hooks.build_instrumentation_hooks(
            otel.tracer, capture_content=ctx.capture_content
        )
...
async def _start_tool(otel, ctx, tool_id="toolu_pending"):
    await _hook(
        ...
        "PreToolUse",
        tool_id,
        {"tool_name": "Bash", "tool_input": {"command": "offline fixture"}},
    )
...
def _tool_result(tool_id, *, is_error=False, content="SDK result"):
    return UserMessage(
        content=[
            ToolResultBlock(tool_use_id=tool_id, content=content, is_error=is_error)
        ]
    )
...
def test_failure_annotation_does_not_take_hook_end_ownership(otel):
    ...
    assert json.loads(span.attributes[GEN_AI_TOOL_CALL_RESULT]) == {
        "error": "tool failed"
    }
...
def test_real_failure_hook_records_error_output_and_closes_once(
    otel, patched_helpers, monkeypatch
):
    async def scenario():
        ctx = otel.context()
...
def test_sdk_tool_result_closes_missing_post_hook_before_cleanup(otel, is_error):
    async def scenario():
        ...
        _reconcile_sdk_message(
            ctx, _tool_result("toolu_pending", is_error=is_error, content=content)
        )
        assert not ctx.active_tool_spans
...
@pytest.mark.parametrize("is_error", [False, True])
def test_hook_result_duplicate_and_late_hook_end_each_invocation_once(
    otel, patched_helpers, monkeypatch, first, is_error
):
    async def scenario():
        ctx = otel.context()
        span = await _start_tool(otel, ctx)
        ...
        hook_data = (
            {"error": "hook failure"} if is_error else {"tool_response": "hook result"}
        )
        ...
        _reconcile_sdk_message(ctx, message)
        _reconcile_sdk_message(
            ctx, _result(permission_denials=[{"tool_use_id": "toolu_pending"}])
        )
        await _hook(
            otel,
            ctx,
            "PostToolUseFailure",
            "toolu_pending",
            {"error": "late contradictory error"},
        )
        ctx.cleanup_unclosed_spans()
        assert len(endings) == 1
...
@pytest.mark.parametrize(
    "outcome", ["success", "tool_error", "permission_denied", "failure_hook"]
)
def test_structure_only_keeps_status_without_sensitive_result_content(
    otel, patched_helpers, outcome
):
    async def scenario():
        ...
        if outcome == "failure_hook":
            await _hook(
                otel, ctx, "PostToolUseFailure", "toolu_pending", {"error": sentinel}
            )
            ...
            _reconcile_sdk_message(
                ctx,
                _tool_result(
                    "toolu_pending", is_error=outcome == "tool_error", content=sentinel
                ),
            )
        assert not span.is_recording()
...
def test_successful_agent_result_does_not_invent_outcomes_for_orphans(otel):
    async def scenario():
        ...
        _reconcile_sdk_message(
            ctx,
            UserMessage(content="Tool permission denied, but no explicit tool result"),
        )
...
def _configure_upstream(monkeypatch, otel):
    ...
    for name, value in {
        ...
        "_token_histogram": meter.create_histogram("tokens"),
        "_duration_histogram": meter.create_histogram("duration"),
    }.items():
...
@pytest.mark.parametrize("is_error", [False, True])
def test_real_wrapper_reconciles_tool_result_before_upstream_cleanup(
    otel, patched_helpers, monkeypatch, mode, is_error
):
    async def scenario():
        upstream = _configure_upstream(monkeypatch, otel)
        previous = otel.context()
        client = SimpleNamespace(
            _otel_invocation_ctx=otel.context(),
            _query=SimpleNamespace(_otel_invocation_ctx=None),
        )
        ...
        iterator = (
            upstream._instrumented_query(
                messages, (), {"prompt": "offline", "options": ClaudeAgentOptions()}
            )
            if mode == "standalone"
            else upstream._instrumented_receive_response(messages, client, (), {})
        )
        ...
        assert (
            json.loads(seen["tool"].attributes[GEN_AI_TOOL_CALL_RESULT]) == "SDK result"
        )
...
def test_wrapper_aclose_finalizes_upstream_and_restores_prior_context(
    otel, patched_helpers, monkeypatch, mode, terminal
):
    async def scenario():
        upstream = _configure_upstream(monkeypatch, otel)
        previous = otel.context()
        ...
        client = SimpleNamespace(
            _otel_invocation_ctx=invocation,
            _query=SimpleNamespace(_otel_invocation_ctx=None),
        )
        _context.set_invocation_context(previous)
        seen = {}
        ...
        if terminal:
            ...
            tool = next(
                span
                for span in otel.recorder.spans
                if span.attributes.get(GEN_AI_TOOL_CALL_ID) == "toolu_orphan"
            )
...
def test_standalone_streaming_prompt_receives_result_before_next_input(
    otel, patched_helpers, monkeypatch
):
    async def scenario():
        upstream = _configure_upstream(monkeypatch, otel)
        ...
        iterator = upstream._instrumented_query(
            messages, (), {"prompt": prompt(), "options": ClaudeAgentOptions()}
        )
...
def test_standalone_delivers_error_result_before_sdk_result_error(
    otel, patched_helpers, monkeypatch
):
    async def scenario():
        upstream = _configure_upstream(monkeypatch, otel)
        ...
        failure = ResultError(
            "offline terminal failure", data={"is_error": True}, exit_code=1
        )
        events = []
        seen = {}
...
def test_error_result_then_immediate_close_preserves_failure_without_tools(
    otel, patched_helpers, monkeypatch, mode, capture_content
):
    async def scenario():
        upstream = _configure_upstream(monkeypatch, otel)
        ...
        previous = otel.context()
        invocation = (
            otel.context(capture_content=capture_content) if mode == "client" else None
        )
        client = SimpleNamespace(
            _otel_invocation_ctx=invocation,
            _query=SimpleNamespace(_otel_invocation_ctx=None),
...
def test_shared_owners_keep_compatibility_wrappers_until_last_deactivation(
    otel, monkeypatch
):
    originals = (
        UpstreamInstrumentor._instrumented_query,
        UpstreamInstrumentor._instrumented_receive_response,
        _hooks.set_tool_error_attributes,
    )
    monkeypatch.setattr(
        _instrumentation.trace, "get_tracer_provider", lambda: otel.provider
    )
    ...
    try:
        ...
        assert all(
            actual is not original for actual, original in zip(wrapped, originals)
        )
        second.activate()
...
@pytest.mark.parametrize("status", ["completed", "failed", "killed", "paused"])
def test_latest_background_task_update_closes_only_correlated_terminal_tool(
    otel, status
):
    ...
    async def scenario():
        ctx = otel.context()
        span = await _start_tool(otel, ctx, "task-tool")
        unrelated = await _start_tool(otel, ctx, "unrelated-tool")
        _reconcile_sdk_message(
            ctx,
            TaskStartedMessage(
        ...
        assert span.is_recording() is (status == "paused")
        assert unrelated.is_recording()
        if status != "paused":
            assert (
                json.loads(span.attributes[GEN_AI_TOOL_CALL_RESULT])["status"] == status
            )
            assert (span.status.status_code is StatusCode.ERROR) is (
                status != "completed"
            )
...
def test_latest_structured_result_and_per_model_usage(
    otel, patched_helpers, capture_content
):
    from opentelemetry.instrumentation.claude_agent_sdk import _spans
    from opentelemetry.instrumentation.claude_agent_sdk._constants import (
    ...
    ctx = otel.context(capture_content=capture_content)
    _context.set_invocation_context(ctx)
    result = _result(
        ...
        model_usage={
            ...
            "claude-sonnet-4-5": {"inputTokens": 3, "outputTokens": 1},
        },
    )
    _spans.set_result_attributes(ctx.invocation_span, result)
    attrs = ctx.invocation_span.attributes
    assert attrs[GEN_AI_USAGE_INPUT_TOKENS] == 12
    assert attrs[GEN_AI_USAGE_OUTPUT_TOKENS] == 5
    if capture_content:
        assert json.loads(
            json.loads(attrs[GEN_AI_OUTPUT_MESSAGES])[-1]["content"][0]["text"]
        ) == {"answer": 42}
    else:
