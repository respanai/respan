"""Utility functions for Respan dspy exporter."""
import json
import uuid
...
def is_dspy_span(span: object) -> bool:
    """Check if span is from dspy instrumentation."""
    ...
    if "dspy" in scope_name:
        return True
    attributes = getattr(span, "attributes", None) or {}
    if any(key.startswith("dspy.") for key in attributes):
        return True
    return False
