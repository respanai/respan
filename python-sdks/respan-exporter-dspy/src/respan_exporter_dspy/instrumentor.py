"""Respan OpenTelemetry redirect for dspy traces."""
...
from respan_exporter_dspy.exporter import RespanDSPyExporter
from respan_exporter_dspy.utils import group_spans_by_trace, is_dspy_span, otel_span_to_dict

logger = logging.getLogger(__name__)

_INSTRUMENTS = ("dspy >= 2.5.0", "openinference-instrumentation-dspy >= 0.1.0")
_PATCHED = False
...
_ACTIVE_EXPORTER: Optional[RespanDSPyExporter] = None
_ACTIVE_DEDUPE = _SpanDedupeCache()
_ACTIVE_PASSTHROUGH = False


def _export_dspy_spans(spans: Iterable[object]) -> SpanExportResult:
    """Export dspy spans to Respan."""
    ...
    if not exporter.api_key:
        logger.warning("Respan API key is not set; skipping dspy export")
        return SpanExportResult.SUCCESS
...
def _batch_export_wrapper(wrapped, instance, args, kwargs):
    ...
    dspy_spans = [s for s in spans if is_dspy_span(span=s)]
    other_spans = [s for s in spans if not is_dspy_span(span=s)]
...
def _on_end_wrapper(wrapped, instance, args, kwargs):
    ...
    if span is None or not is_dspy_span(span=span):
        return wrapped(*args, **kwargs)

    try:
        _export_dspy_spans(spans=[span])
    except Exception as exc:
        logger.warning("Failed to export dspy span: %s", exc, exc_info=True)
...
class RespanDSPyInstrumentor(BaseInstrumentor):
    """Instrument OTel exporters to send dspy traces to Respan."""

    def __init__(self) -> None:
        super().__init__()
        self._exporter: Optional[RespanDSPyExporter] = None
    ...
    def _instrument(self, **kwargs) -> None:
        self._exporter = RespanDSPyExporter(
        ...
        logger.info("Respan dspy instrumentation enabled")
    ...
    def _uninstrument(self, **kwargs) -> None:
        ...
        logger.info("Respan dspy instrumentation disabled")
    ...
    def _patch_span_processors(self) -> None:
        ...
        _PATCHED = True
        logger.debug("Patched OTel span processors for dspy export")
