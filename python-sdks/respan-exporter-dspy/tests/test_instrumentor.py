"""Tests for dspy OTel instrumentor."""
...
class TestRespanDSPyInstrumentor:
    """Test instrumentor setup."""

    def test_instrumentation_dependencies(self):
        from respan_exporter_dspy.instrumentor import RespanDSPyInstrumentor

        instrumentor = RespanDSPyInstrumentor()
        deps = instrumentor.instrumentation_dependencies()
        assert any("dspy" in dep for dep in deps)
        assert any("openinference-instrumentation-dspy" in dep for dep in deps)
    ...
    def test_instrument_creates_exporter(self):
        ...
        with patch("respan_exporter_dspy.instrumentor.RespanDSPyExporter") as mock_cls:
            with patch.object(instrumentor, "_patch_span_processors"):
                instrumentor._instrument(api_key="test-key", environment="staging")
                mock_cls.assert_called_once()


class TestExportDSPySpans:
    """Test span export function."""

    def test_filters_non_dspy_spans(self):
        from respan_exporter_dspy import instrumentor as mod

        non_dspy_span = SimpleNamespace(
        ...
        try:
            mod._export_dspy_spans(spans=[non_dspy_span])
            mock_exporter.build_payload.assert_not_called()
        finally:
            mod._ACTIVE_EXPORTER = original

    def test_exports_dspy_spans(self):
        from respan_exporter_dspy import instrumentor as mod
        ...
        mod._ACTIVE_EXPORTER = mock_exporter
        try:
            mod._export_dspy_spans(spans=[span])
...
class TestSpanDedupeCache:
    ...
    def test_deduplicates_same_span(self):
        from respan_exporter_dspy.instrumentor import _SpanDedupeCache
