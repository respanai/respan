"""Respan microsoft_agents Exporter - Export microsoft_agents traces to Respan."""
import logging
import os
import uuid
...
from respan_exporter_microsoft_agents.types import TraceContext
from respan_exporter_microsoft_agents.utils import (
    as_dict,
    clean_payload,
    coerce_datetime,
    coerce_token_count,
...
class RespanMicrosoftAgentsExporter:
    """Export microsoft_agents traces/spans to Respan."""
    ...
    def _extract_trace_context(self, trace_obj: Optional[Any], spans: Sequence[Any]) -> TraceContext:
        ...
        if not trace_name:
            trace_name = pick_metadata_value(root_metadata, "microsoft_agents.agent.type", "agent.name")
        if not trace_name:
    ...
    def _span_to_respan(self, span: Any, trace_context: TraceContext, span_id_map: Optional[Dict[str, str]] = None) -> Optional[Dict[str, Any]]:
        ...
        if "microsoft_agents_trace_id" not in merged_metadata:
            merged_metadata["microsoft_agents_trace_id"] = trace_context.trace_id
        if "microsoft_agents_span_id" not in merged_metadata:
            merged_metadata["microsoft_agents_span_id"] = str(span_id)
        if parent_id and "microsoft_agents_parent_id" not in merged_metadata:
            merged_metadata["microsoft_agents_parent_id"] = str(parent_id)
        ...
        try:
            ...
            logger.warning("microsoft_agents span payload failed validation: %s", exc)
