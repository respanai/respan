"""Respan OpenTelemetry redirect for Google ADK traces.
...
from respan_exporter_google_adk.utils import group_spans_by_trace, is_adk_span, otel_span_to_dict
...
def _export_adk_spans(spans: Iterable[object]) -> SpanExportResult:
    ...
    if not exporter.api_key:
        logger.warning("Respan API key is not set; skipping ADK export")
        return SpanExportResult.SUCCESS
...
class RespanGoogleAdkInstrumentor(BaseInstrumentor):
    """Instrument OpenTelemetry exporters to send Google ADK traces to Respan."""

    def __init__(self) -> None:
        super().__init__()
        self._exporter: Optional[RespanGoogleAdkExporter] = None
    ...
    def _instrument(self, **kwargs) -> None:
        self._exporter = RespanGoogleAdkExporter(
